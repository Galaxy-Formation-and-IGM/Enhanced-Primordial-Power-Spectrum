...
def P(k, z, cutoff=np.inf):
    ...
    return np.where(k <= cutoff, Pk, 0.0)
...
def sigma_R(R, z=0, cutoff=np.inf):

    # Integrate over log10(k)
...
A = cosmo.sigma8_obs / sigma_R(R8, 0, np.inf)
...
M_range = np.logspace(2, 11, 1000)

# Extra addition:

Standard_Sigma_Norm = [
    A * sigma_M(M, 0, np.inf)
    for M in M_range
]
...
logM = np.log10(M_range)
logsigma = np.log10(Standard_Sigma_Norm)

sigma_spline = CubicSpline(logM, logsigma)

sigma_fit = 10**sigma_spline(logM)

def sigma_fit_standard(M):
    return 10**sigma_spline(np.log10(M))

sigma_spline_deriv = sigma_spline.derivative()

def dlnsigma_dlnM(M):
    return sigma_spline_deriv(np.log10(M))

"""
    
def PS_MassFunc_Multiplicity(M, z):
    # Generate the fitted polynomial function
...
def PS_MassFunc(M, z):
    return PS_MassFunc_Multiplicity(M, z) * cosmo.rho_0 / M

"""

def PS_MassFunc(M, z):      # Returns dn/dlog10(M)

    sigma = sigma_fit_standard(M) * D(z)
